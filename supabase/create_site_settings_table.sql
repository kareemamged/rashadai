...
CREATE POLICY IF NOT EXISTS site_settings_admin_select_policy ON site_settings
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS site_settings_admin_update_policy ON site_settings
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM admin_users
      WHERE admin_users.id = auth.uid()
...
CREATE POLICY IF NOT EXISTS site_settings_admin_insert_policy ON site_settings
...
CREATE POLICY IF NOT EXISTS site_settings_auth_select_policy ON site_settings
  FOR SELECT USING (auth.role() = 'authenticated');

-- Create policy for anonymous users to select site settings
CREATE POLICY IF NOT EXISTS site_settings_anon_select_policy ON site_settings
